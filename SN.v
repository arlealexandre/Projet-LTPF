...
Definition S2 := [false; true].
Definition S3 := [false; false].
Definition S4 := [true; true; false; true].
...
(** On code un programme P1 correspondant à
    while (i=false) do {i:=not false;j:=false} *)
Definition I := 1.
Definition J := 0.
Definition V := Bnot Bfalse.

Definition corps_boucle := Seq (Assign I V) (Assign J Bfalse).
Definition P1 := While (Bnot (Bva I)) corps_boucle.
